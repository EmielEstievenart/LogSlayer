...
- **`code/LogSlayer/core/`** → `logslayer_core` (static lib). The UI-framework-free heart: watchers, tracked/processed sources, sorting/merging, filtering, searching (`search_pattern`, `log_view2_find_manager`), timestamps, settings, clipboard, command-line parsing, the notification *interface*, the command *model* (`CoreCommand`, `CommandManager`, `CommandContext`, `command_palette_model`, `command_history`, and the ~20 pure command actions under `commands/implementations/`), plus the `LogViewService` abstraction and the `AlignTimeSession` alignment model. **Links no FTXUI** — any `<ftxui/...>` or `<ftxui_components/...>` include in a core file is a hard compile error, which is what enforces portability.
- **`code/LogSlayer/ui/`** → `logslayer_ui` (static lib; depends on `logslayer_core` + FTXUI). The terminal UI: the LogView2 render stack (`LogView2/`, including the dual-pane `align_time_controller`), the command palette controller/view + widgets, the interactive (picker) commands, the command registrar, the toast sink, the colour theme (`view_theme`), and `LogView2Bridge` (the FTXUI implementation of the core `LogViewService`). The legacy LogView1 stack has been deleted; `docs/logview1-vs-logview2.md` records the migration.
...
**The log view.** `main.cpp` shows a single fullscreen `LogView2Component` (`ui/LogView2/`) backed by an `AllProcessedSourcesLogView2Data` adapter over the one `AllProcessedSources` model, with one command palette. `LogView2Data` is a small read-only interface (`size`/`to_string`/`widest_line_width`/`lock`/callbacks); the concrete adapter is what makes the read/write boundary explicit (the view can't mutate the model). LogView2 is the sole view: all commands are registered for it via `register_log_view2_commands`, and it owns find, the dual-pane time-alignment mode, and the bottom status bar. (`docs/logview1-vs-logview2.md` is the historical LogView1→LogView2 migration record.)

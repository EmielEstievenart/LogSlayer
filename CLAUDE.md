...
**Threading.** `main.cpp` spawns one background watcher thread (`start_watcher_thread`) that sleeps `poll_interval_ms`, calls `tracked_sources.poll()`, and on new data updates `AllProcessedSources` (append/replace) and posts a custom FTXUI event to redraw (the pull-based view re-renders from the model). A single `model_mutex` guards the whole model (tracked sources, processed sources); the view reads it through `LogView2Data::lock`. **Any code reading or mutating the model must hold `model_mutex`** — commands receive it via `CommandContext::model_mutex`. The TSan preset exists to catch violations.

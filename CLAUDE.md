...
**Session configs / replay** — the session is snapshot-serializable as an ordered list of palette command lines, which is the single canonical format behind `--cmd`, saved configs, and script export. `session_snapshot` (core) serializes model state (opens with absolutized paths → per-source `set-time-format`/`set-offset` → filters → `hide-columns` → both display toggles explicitly → `hide-before-line` → `go-to-line` on the viewport-center line); `session_replay` (core) executes such a list, routing open/open-file/open-folder through the commands' *synchronous* path (a null-mutex context copy) so later lines can reference the source they open, and cancelling+reporting any command that starts a palette interaction. Consumers: `[config:<name>]` sections in the settings ini (`save-config`/`load-config`/`list-configs`/`delete-config` via `session_config_store`; `load-config` = `reset-session` + replay, guarded against recursive load-config), script export (`export-config <path>.bat|.cmd|.sh` via `session_script` — CRT-quoted with `%%` escaping for batch; no path → clipboard one-liner), and the CLI (repeatable `--cmd "<palette line>"`, `--config <name>`, `--resume-last`). `main.cpp` replays startup commands after `register_log_view_commands`, before `screen.Loop`, under `model_mutex` (failures toast and continue; positional paths keep their strict abort), and auto-saves `[config:__last]` after the loop exits and threads are joined. Textual source-addressing commands (`set-offset`, `clear-offset`, `set-time-format <source> <format|auto>`, `reset-time-format`, `close`) resolve the source by path/URI (normalized identity) or mnemonic via `source_resolver`; quoted first arguments parse through `command_arguments` (`""` = literal quote). Offsets round-trip through `serialize_log_timestamp_offset` (the parseable form; `format_log_timestamp_offset` stays display-only).

**Settings** (`settings_store`, `settings_ini`) — INI-backed (simpleini) persistence for settings, command history, timestamp formats, and session configs. Platform-specific path resolved by `default_settings_file_path()` (LOCALAPPDATA / Application Support / XDG_CONFIG_HOME, with a CWD fallback). Settings loading is resilient: failures disable saves for the run rather than aborting.

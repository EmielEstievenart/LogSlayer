...
CommandResult GoToLineCommand::execute(std::string_view arguments)
{
    ...
    if (!_context.log_controller.go_to_line(_context.processed_sources, *line_number)) { return {false, "Line " + std::to_string(*line_number) + " is hidden by current filters or line cutoff"}; }

...
class LambdaCommand final : public CoreCommand
{
public:
...
void CommandManager::register_command(std::unique_ptr<CoreCommand> command)
{
    if (command == nullptr)
    {
...
CoreCommand* CommandManager::active_command()
{
    return _active_command;
}

const CoreCommand* CommandManager::active_command() const
{
    return _active_command;
}

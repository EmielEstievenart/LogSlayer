...
CommandResult FindCommand::execute(std::string_view arguments)
{
    ...
    try { focused_visible_match = _context.log_view.set_find_query(_context.processed_sources, query); }
    catch (const std::invalid_argument& error) { return {false, "Invalid find pattern: " + std::string(error.what())}; }

    const int visible_matches = _context.log_view.visible_find_match_count(_context.processed_sources);
    const int total_matches = _context.log_view.total_find_match_count();
    if (focused_visible_match)
    {
        return {true, "Find active: " + _context.log_view.find_query() + " (" + std::to_string(visible_matches) + " visible / " + std::to_string(total_matches) + " total)"};
    }

    return {true, "Find active: " + _context.log_view.find_query() + " (0 visible / " + std::to_string(total_matches) + " total)"};
}

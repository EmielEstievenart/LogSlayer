...
| wxWidgets vendoring | Git submodule `libs/wxwidgets` (https URL), pinned to the newest 3.3.x tag (native Windows dark mode, current-toolchain support; the pin controls API drift). Fallback if 3.3 fails on the clang preset: newest 3.2.x tag; if clang still fails, wx development uses the msvc preset and the limitation is documented here. Static wx (`wxBUILD_SHARED=OFF`), dynamic CRT (repo default), samples/tests/demos off — matching the established submodule pattern in the root CMakeLists. |

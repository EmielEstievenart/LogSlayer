...
- [x] M4 — the five interactive picker commands (delete-filters, close-open-file, set-time-format, adjust-time-offset, clear-time-offset) rewritten as stateless core `CoreCommand`s that drive the `CommandPaletteSession` picker modes (`(CommandContext, CommandPaletteSession&)`); the FTXUI widget implementations and the `ViewPickerCommandFactory` slot seam deleted — `register_core_view_commands(manager, context, session)` now registers the full shared set in the pinned TUI palette order for both UIs; `source_labels_with_offsets` moved to core `command_support`; LV2 `open` registered for wx too (it was already a pure core command); wx palette gained the picker-mode chrome (sub-headers, per-mode key hints, Space toggle, offset preview status, key gating); 9 new end-to-end picker tests, 233 total green (2026-07-02)
- [ ] M5
- [ ] M6
- [ ] M7
- [ ] M8
